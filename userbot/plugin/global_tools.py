...
@userbot_cmd(pattern="gpro(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    userbot = await eor(event, "`Promoting globally...`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await userbot.edit("You can't promote yourself...")
       return
    try:
        if not rank:
            rank = "ㅤ"
    except:
        return await userbot.edit("**ERROR !!**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await userbot.edit(f"**Promoting User in :**  `{i}` Chats...")
          except:
             pass
    else:
...
@userbot_cmd(pattern="gdem(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    userbot = await eor(event, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await event.get_chat()
    if event.is_private:
    ...
    try:
        ...
        pass
    if me == user:
       k = await userbot.edit("You can't Demote yourself !!")
       return
    try:
        if not rank:
            rank = "ㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await event.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await userbot.edit(f"**Demoting Globally In Chats :** `{i}`")
          except:
             pass
    else:
        await userbot.edit(f"**Reply to a user !!**")
...
@userbot_cmd(pattern="gban(?:\s|$)([\s\S]*)")
async def _(event):
    userbot = await eor(event, f"`Gbanning ...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(userbot, "**To gban a user i need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == ForGo10God:
        return await eod(userbot, "🥴 **Nashe me hai kya lawde ‽**")
    if str(userid) in DEVLIST:
        return await eod(userbot, "😑 **TERI MAA KO CHODA H KY ISNE ?¿ Really‽**")
    if is_gbanned(userid):
        return await eod(
            userbot,
            "ISKI MAIYA TO PHLE HI CHUDI PDI H CHECK MY **Gban LIST!!**",
        )
    ...
    if a is not None:
        ...
        gbpic = cjb
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **beta majdur ko khodna 😪 aur** {userbot_mention} **ko chodna... Kabhi sikhana nhi!! 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {userbot_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
        ...
        await userbot.edit(ogmsg)


@userbot_cmd(pattern="ungban(?:\s|$)([\s\S]*)")
async def _(event):
    cid = await client_id(event)
    ForGo10God, USERBOT_USER, userbot_mention = cid[0], cid[1], cid[2]
    userbot = await eor(event, "`Ungban in progress...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(userbot, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(userbot, "`User is not gbanned.`")
    await userbot.edit(f"Ungbaning in client 1... \n**Unbanned in :** `{chats}`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            userbot = int(hel)
            try:
                tity = await event.client.get_entity(userbot)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={userbot}) (`{userbot}`)\n"
    else:
...
@userbot_cmd(pattern="gkick(?:\s|$)([\s\S]*)")
async def gkick(event):
    cid = await client_id(event)
    ForGo10God, USERBOT_USER, userbot_mention = cid[0], cid[1], cid[2]
    userbot = await eor(event, f"`Kicking globally ...`")
    reply = await event.get_reply_message()
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(userbot, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == ForGo10God:
        return await eod(userbot, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eod(userbot, "**😪 ISNE TERI MAIYA KO CHODA NA HOTA TO TUJ JESA PAIDA NA HOTA!!**")
    ...
    if Config.ABUSE == "ON":
        ...
        await userbot.edit(gkmsg)


@userbot_cmd(pattern="gmute(?:\s|$)([\s\S]*)")
async def gm(event):
    private = False
    cid = await client_id(event)
    ForGo10God, USERBOT_USER, userbot_mention = cid[0], cid[1], cid[2]
    ...
    if str(userid) in DEVLIST:
        return await eod(event, "**TEREKO KITNI BAAR BOLA BAAP SE BAKCHODI NHI..**")
    try:
...
@userbot_cmd(pattern="ungmute(?:\s|$)([\s\S]*)")
async def endgmute(event):
    ...
    if not gsql.is_gmuted(userid, "gmute"):
        return await eod(event, "ISKA MUH KAB CHODA...")
    try:
