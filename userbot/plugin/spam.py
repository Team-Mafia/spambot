...
• `{i}spam <no of msgs> <your msg>`
  `{i}spam <no of msgs> <reply message>`
    spams chat, the current limit for this is from 1 to 999.

• `{i}bigspam <no of msgs> <your msg>`
  `{i}bigspam <no of msgs> <reply message>`
    Spams chat, the current limit is above 1000.
...
@userbot_cmd(pattern="bigspam", fullsudo=True)
async def bigspam(e):
    message = e.text
    if e.reply_to:
        if not len(message.split()) >= 2:
    ...
    try:
        counter = int(counter)
            if counter >= 100000:
            return await eod(e, "`this is the maximum limit`")
    except BaseException:
        return await eod(e, "`Use in Proper Format`")
    await asyncio.wait([e.respond(spam_message) for i in range(counter)])
    await e.delete()
