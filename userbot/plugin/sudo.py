...
• `{i}listsudo`
    List all sudo users.
"""
from userbot.sql import add_sudo, del_sudo, is_sudo


@userbot_cmd(pattern="addsudo ?(.*)", fullsudo=True)
async def _(ult):
...
@userbot_cmd(pattern="delsudo ?(.*)", fullsudo=True)
async def _(ult):
    inputs = ult.pattern_match.group(1)
    if ult.reply_to_msg_id:
        replied_to = await ult.get_reply_message()
...
async def _(ult):
    ...
    if m == "False":
        m = "[False](https://telegra.ph/Userbot-04-06)"
    return await eor(
