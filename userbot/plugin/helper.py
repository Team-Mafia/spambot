...
name = f"{eagle_mention}'s Assistant"

file = "https://telegra.ph/file/3d6ea2ded6f1a3f95765e.jpg"
description = (
    f"I am Assistant Of {eagle_mention}.This Bot Can Help U To Chat With My Master"
)
...
async def repo(event):
    try:
        eagle = await bot.inline_query(botname, "repo")
        await eagle[0].click(event.chat_id)
        if event.sender_id == MAMBA_STAR:
            await event.delete()
    except (noin, dedbot):
        await eor(event, msg)
        ...
        try:
            ...
            async with bot.conversation(chat) as conv:
                try:
                    ...
                    return await eagle.edit("Unblock @Botfather first.")
                await legend.edit(
...
async def eaglemafia-spamuserbott(event):
    if event.fwd_from:
        return
